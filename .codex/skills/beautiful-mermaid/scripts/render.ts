  ...
  try {
    ...
    try {
      if (runtime === "deno") {
        ...
        const cacheDir =
          process.env.BEAUTIFUL_MERMAID_CACHE_DIR ??
          resolve(homedir(), ".cache", "beautiful-mermaid-skill");
        mkdirSync(cacheDir, { recursive: true });
        if (runtime === "bun") {
          execSync(`bun add --cwd "${cacheDir}" ${name}`, { stdio: "inherit" });
        } else {
          execSync(
            `npm install --prefix "${cacheDir}" --no-save --no-package-lock ${name}`,
            { stdio: "inherit" },
          );
        }
        const resolvedPath = require.resolve(name, { paths: [cacheDir] });
